...
-- Alterar para recuperação SIMPLE (uso de string dinâmica)
EXEC ('ALTER DATABASE [' + @DBNAME + '] SET RECOVERY SIMPLE');

-- Obter o nome lógico do arquivo de log
SELECT @LogFileName = name
FROM sys.master_files
WHERE database_id = DB_ID(@DBNAME) AND type_desc = 'LOG';

-- Encolher o arquivo de log (usando o nome lógico)
DBCC SHRINKFILE (@LogFileName, 1);

-- Encolher o banco de dados (opcional e com cautela)
DBCC SHRINKDATABASE(@DBNAME);

-- Modificar o arquivo de log (crescimento automático e tamanho máximo)
EXEC('ALTER DATABASE [' + @DBNAME + '] MODIFY FILE 
(
    NAME = ''' + @LogFileName + ''',
    FILEGROWTH = 15%,
    MAXSIZE = UNLIMITED
...
/****** COMPACTAÇÃO DE LOG E LIBERAÇÃO DE BUFFER PÓS REINDEXAÇÃO ******/
DECLARE @DBNAME SYSNAME;
DECLARE @LogFileName NVARCHAR(128);
